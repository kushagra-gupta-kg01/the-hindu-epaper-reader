...
├── requirements.txt            # Python requirements (e.g., fastapi, aiofiles, requests, bs4, pytest-cov)
├── src
│   ├── __init__.py
...
```bash
PYTHONPATH=. .venv/bin/pytest --cov=src --cov=api --cov-report=term-missing tests/
```
